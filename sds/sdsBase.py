...
class sdsJsonEncoder(json.JSONEncoder):
    """
    Custom JSON encoder for sds,
...
def endpoint(path, methods):
    """
    Specilaized decorator to use in the methods of the class that inherit from  sdsPluginBase
    this decorator allows to register the path and methods [GET,POST,DELETE,PUT]
    in the flask app.

    example:
    class Hello(sdsPluginBase):
...
class sdsPluginBase:
    def __init__(self, sds):
        ...
        self.apikey = sds.apikey
        self.app = sds.app
        self.request = request
        self.json = json
        self.logger = sds.logger
        self.sds = sds
